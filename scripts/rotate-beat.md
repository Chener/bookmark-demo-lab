...
5. **X ingest（Firstmate X MCP slim）**：刚结束窗 `[opensAt, closesAt)` 的书签增量。优先 `bookmarked_at` / `bookmarkedAt`，否则 `created_at`。跳过 `tracking/seen-bookmarks.json` 已有 id。无 slim 文件则 `candidates=[]`，绝不伪造。
...
`--ingest-only` 按当前窗 `[opensAt, closesAt)` 重写 `candidates` 与 `ingestNoteZh`。slim 文件缺失时保持原候选不动。同步枢纽仍需管理员 `PUT /api/window`。
...
python3 scripts/rotate-beat.py --ingest-only --dry-run
python3 scripts/rotate-beat.py --ingest-only   # 只刷新当前窗候选，不 settle

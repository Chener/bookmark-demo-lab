(() => {
  const MODELS = [
    {
      ...
      layers: "48 × 多头注意力",
      context: "1,024",
      kv: "约 300 KiB / 词元 · 半精度粗算",
      summary: "2019 年的对照原点：随机失活、高斯误差激活、层归一化，完整多头注意力。后来几乎所有新卡都在改这三块积木。",
      bullets: [
        "词嵌入与位置嵌入分开相加，上下文很短。",
        "每个块都是层归一化 → 注意力 → 残差 → 层归一化 → 前馈。",
        "没有分组查询，也没有旋转位置，键值缓存相对偏贵。",
      ],
      stack: [
        ["embed", "词嵌入"],
        ["pos", "绝对位置"],
    ...
    },
    {
      ...
      attnColor: "#db2777",
      decoder: "稠密解码器",
      attention: "分组查询注意力 · 旋转位置",
      layers: "32 × 分组查询",
      context: "8,192",
      kv: "约 128 KiB / 词元 · 半精度粗算",
      summary: "当代稠密栈的基准卡：预归一化、旋转位置、分组查询，宽度比同规模 OLMo 2 更大。",
      bullets: [
        "均方根归一取代层归一化，门控前馈取代高斯误差激活。",
        "分组查询让键值头少于查询头，推理缓存立刻变轻。",
        "适合拿来对照后续的 查询键归一化、滑动窗口和专家路由。",
      ],
      stack: [
        ["embed", "词嵌入"],
        ["pos", "旋转位置"],
    ...
    },
    {
      ...
      attnColor: "#7c3aed",
      decoder: "稠密解码器",
      attention: "多头注意力 · 查询键归一化",
      layers: "32 × 多头注意力",
      context: "4,096",
      kv: "约 512 KiB / 词元 · 半精度粗算",
      summary: "透明开源对照：保住经典多头，却把归一化挪进残差内部，训练更稳、缓存更重。",
      bullets: [
        "不是常见的预归一化，而是残差内部的后归一化。",
        "查询键再做一次归一化，注意力数值更稳。",
        "完整多头让键值体积明显高于同规模分组查询模型。",
      ],
      stack: [
        ["embed", "词嵌入"],
        ["pos", "旋转位置"],
    ...
    },
    {
      ...
      layers: "16 × 分组查询",
      context: "128,000",
      kv: "约 32 KiB / 词元 · 半精度粗算",
      summary: "小参数、长上下文：层数更少、隐藏维更宽，常被拿来和 Qwen3 小卡对着看。",
      bullets: [
        "只有 16 层，但上下文直接拉到 128K。",
        "键值缓存很轻，适合本机试结构而不是冲分数。",
        "配方仍是 Llama 家族的均方根归一、分组查询与旋转位置。",
      ],
      stack: [
        ["embed", "词嵌入"],
        ["pos", "旋转位置"],
    ...
    },
    {
      ...
      attnColor: "#0284c7",
      decoder: "稠密解码器",
      attention: "分组查询 · 查询键归一化",
      layers: "36 × 分组查询",
      context: "32,768",
      kv: "约 144 KiB / 词元 · 半精度粗算",
      summary: "更深更窄的小稠密卡：查询键归一化加上约 15 万词表，是看 Qwen3 家族的入门积木。",
      bullets: [
    ...
    },
    {
      ...
      layers: "61 × 潜在注意力",
      context: "128,000",
      kv: "约 68.6 KiB / 词元 · 半精度粗算",
      summary: "近年开源混合专家的模板卡：潜在注意力压缓存，共享专家加稠密前缀，再加多词预测。",
      bullets: [
        "潜在注意力把键值压进压缩向量，长上下文才扛得住。",
        "路由专家旁边永远有一条共享专家。",
        "训练时的多词预测让推测解码更顺，结构上也多出一条头。",
      ],
      stack: [
        ["embed", "词嵌入"],
        ["pos", "旋转位置"],
    ...
    },
    {
      ...
      context: "128,000",
      kv: "约 68.6 KiB / 词元 · 半精度粗算",
      summary: "把 DeepSeek 配方再放大：专家更多、潜在注意力头更少，激活参数仍控制在可服务的量级。",
      bullets: [
        "总参数跨到万亿，激活大约 32B。",
        "注意力仍是潜在压缩，所以键值缓存几乎和 V3 同级。",
    ...
    },
    {
      ...
      layers: "40 × 分组查询",
      context: "128,000",
      kv: "约 160 KiB / 词元 · 半精度粗算",
      summary: "为延迟改过的 24B：丢掉旧版滑动窗口，层数比 Gemma 3 27B 更少，键值缓存也更克制。",
      bullets: [
        "注意力回到朴素分组查询，结构非常好认。",
    ...
    },
    {
      ...
      layers: "36 分块 + 12 全量",
      context: "1,000,000",
      kv: "约 192 KiB / 词元 · 半精度粗算",
      summary: "Meta 的大混合专家：专家更少更大，稠密块与专家块交替，上下文直接拉到百万级。",
      bullets: [
        "注意力在分块局部和全量分组查询之间切换。",
        "专家数量少于 DeepSeek，单个专家更宽。",
        "看这张卡能立刻感到「混合专家也可以很传统」。",
      ],
      stack: [
        ["embed", "词嵌入"],
        ["pos", "旋转位置"],
    ...
    },
    {
      ...
      layers: "94 × 分组查询",
      context: "128,000",
      kv: "约 188 KiB / 词元 · 半精度粗算",
      summary: "贴近 DeepSeek 的稀疏 Qwen：更深、不设共享专家，用路由效率换服务成本。",
      bullets: [
        "94 层把专家卡做得很深。",
        "没有共享专家，所有容量都交给路由。",
        "注意力仍是分组查询而不是潜在注意力，键值缓存比 V3 更重。",
      ],
      stack: [
        ["embed", "词嵌入"],
        ["pos", "旋转位置"],
    ...
    },
    {
      ...
      layers: "36 × 分组查询",
      context: "131,072",
      kv: "约 72 KiB / 词元 · 半精度粗算",
      summary: "小模型里的结构实验：每隔四层摘掉旋转位置，看「无位置层」会不会让长度外推更稳。",
      bullets: [
        "主体仍是分组查询稠密栈。",
        "每四层出现一次无位置编码，积木上会缺一块位置色。",
        "键值缓存很低，适合把结构差异摆在桌面上讲。",
      ],
      stack: [
        ["embed", "词嵌入"],
        ["pos", "旋转 / 间歇无位置"],
    ...
    },
    {
      ...
      moe: true,
      decoder: "稀疏混合专家",
      attention: "交替窗口 / 全局分组查询",
      layers: "12 窗口 + 12 全局",
      context: "128,000",
      kv: "约 48 KiB / 词元 · 半精度粗算",
      summary: "OpenAI 开源小混合专家：更宽更浅，窗口与全局一层隔一层，还带注意力偏置和汇点。",
      bullets: [
        "激活比例高（约 17%），和小而密的混合专家更像。",
        "交替注意力让键值缓存压到很低。",
    ...
    },
    {
      ...
      layers: "92 × 分组查询",
      context: "128,000",
      kv: "约 368 KiB / 词元 · 半精度粗算",
      summary: "代理向混合配方：前三层稠密、其后混合专家，共享专家加上训练期多词预测。",
      bullets: [
        "开头几层不路由，先把表示打稳。",
        "共享专家始终在线，和 DeepSeek 同一思路。",
        "键值缓存偏高，因为注意力没有改成潜在压缩。",
      ],
      stack: [
        ["embed", "词嵌入"],
        ["pos", "旋转位置"],
  ...
  function openModel(id) {
    ...
    document.querySelector("[data-spec]").innerHTML = `
      ...
      <dt>上下文</dt><dd>${model.context} 词元</dd>
      <dt>缓存量级</dt><dd>${model.kv}</dd>
    `;
